...
export class ImportExcel {
    ...
    _originMapData = null // 原始配置
    _mapData = {} // 解析后的数据
    _map = {} // 映射关系
    _keys = [] // 解析后的 keys
    _workbook = null // Excel 对象
    _worksheet = [] // 读取的 Excel 数据
    _onRowLoad = null // 监听行变化时的回调
    ...
    constructor(mapData, options = {}) {
        this._init(mapData, options)
        this._workbook = new ExcelJS.Workbook()
    }

    _init(mapData, options) {
        ...
        this._originMapData = mapData
        this._options.trim = trim
        this._options.onRowLoad = onRowLoad
     ...
     */
    _v1Parse() {
        for (const k in this._originMapData) {
            if (!this._originMapData.hasOwnProperty(k)) continue

            let val = this._originMapData[k]
            ...
            let { key, value, trim = this._options.trim } = val
            ...
            if (typeof value === 'function') {
                ...
                const func = value
                value = async (context) => {
                    this._value(undefined, context)
                    await func(context)
                }
            } else {
                // 非函数, 包装成函数
                value = this._value.bind(this, value)
            }
     ...
     */
    _v2Parse() {
        for (let i = 0; i < this._originMapData.length; i++) {
            const item = this._originMapData[i]
            if (isType(item) !== 'object') {
                // 非对象抛出错误
                throw new TypeError('the projects of "mapData" must all be objects')
            }

            let { originKey, key, value, trim = this._options.trim } = item
    ...
    _verifyFile(file) {
        ...
        const fileType = this._getSuffix(file.name)
        if (fileType !== 'xlsx') {
            return {
                code: -1,
    ...
    async _getData(buf) {
        try {
            const sheel = await this._workbook.xlsx.load(buf)
            const worksheet = sheel.getWorksheet(1)
            this._worksheet = []
            worksheet.eachRow((row) => {
                this._worksheet.push(row.values)
            })
    ...
    async load(file, len = 2) {
        const checking = this._verifyFile(file)
        if (checking.code !== 1) {
            throw checking
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            fileReader.onload = async () => {
                try {
                    await this._getData(fileReader.result)

                    let data = []
                    // 去除首列空洞项
                    this._worksheet.forEach(item => {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        ...
                        for (let j = 0; j < this.keys.length; j++) {
                            ...
                            const { trim, key: originKey, value: handle } = this._mapData[key]
                            const value = trim ? this._trim(item[j]) : item[j]
                            await handle({
                                row: i, // 当前数据所在行下标
                                index: j, // 当前数据的下标
                                ...
                                setData: this._setData.bind(this, rowData)
                            })
                        }
                        ...
                        // 监听行变化时
                        this._onRowLoad && await this._onRowLoad({
                            row: i, // 当前数据所在行下标
                            originRow: i + len, // 当前数据在 Excel 中的行
                            get rowItem() {
                                return clone(item) // 当前行解析前的数据
